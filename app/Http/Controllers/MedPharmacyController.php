...
class MedPharmacyController extends Controller
{
    ...
    public function notification(Request $request){
        ...
        ->select('medicines.name_med','medicines.image','med_pharmacies.quantity','medicines.descrption','medicines.mg',
        'medicines.exp')->get();
         if($med){
            return $med ;
         }
    ...
    public function category(Request $request, MedPharmacy $medPharmacy)
    {
       return MedPharmacy::join('medicines','medicines.id','=','med_pharmacies.med_id')
       ->join('categories','categories.id','=','medicines.category_id')->where('ph_id',$request->id)
       ->where('name_category',$request->name)->select('medicines.id','medicines.name_med','medicines.image',
       'medicines.descrption','medicines.mg','medicines.exp','medicines.price_pharmacy',
       'medicines.price_customer','medicines.status','med_pharmacies.quantity','categories.name_category')->get();
    }
    /**
...
{   
   ...
   ->where('pharmacies.city',$request->city)
   ->select('pharmacies.name_ph','pharmacies.id','pharmacies.phone','pharmacies.city',

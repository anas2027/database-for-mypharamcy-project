...
class WarehouseController extends Controller
{
    ...
    // get most warehouse have order
    public function count(Warehouse $warehouse , Request $request)
    {
      //  $Warehouse = Warehouse::all();
      //  foreach ($Warehouse as  $row) {
      //   $order = Order::whereIn('id_warehouse',[$row->id])->count();

      // //  return $result=['Warehouse'=>$warehouse];
      //   $result=['Warehouse'=>$Warehouse ,['order'=>['count'=>$order]]];

      // }
      
     $orders= Order::select('id_warehouse')->where('status',1)->groupBy('id_warehouse')
    ->orderByRaw('COUNT(*) DESC')
    ->limit(1)
    ->first();

    return Warehouse::where('id',$orders->id_warehouse)->get();
    ...
    public function search(Request $request){
     return Warehouse::where('name_warehouse',$request->name)->get();
    }
}

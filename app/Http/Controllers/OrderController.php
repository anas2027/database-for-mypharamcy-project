...
//...............show orders for pharmacy
    public function showorders(Request $request)
    {
       ...
       ->where('orders.id_ph',$request->id)->select('orders.id_ph','orders.id','orders.id_warehouse','warehouses.name_warehouse',
    ...
    // most user buy from warehouse
    public function customer(Warehouse $warehouse , Request $request)
       {
        $orders= Order::select('id_ph')->where('status',1)->where('id_warehouse',$request->id)->groupBy('id_ph')
       ->orderByRaw('COUNT(*) DESC')
       ->limit(1)
       ->first();
   
       return Pharmacy::where('id',$orders->id_ph)->get();
